...
class SelfState():
    def __init__(self):
        ...
        self.prio_score: list[int] = [1, 5, 6, 3, 4, 2]

        self.base_prio: list[int] = [0, 5, 3, 4, 1, 2] # Continent priority (1st index highest prio)
...
def handle_claim_territory(game: Game, self_state: SelfState, bot_state: BotState, query: QueryClaimTerritory) -> MoveClaimTerritory:
    ...
    """
    continent_territories = get_continent_territory(game, self_state) # All continent territory in owned continents
    continent_inchoke = []
    for cont in conts:
        continent_inchoke = continent_inchoke + self_state.in_choke_points[cont]

    #print('A', flush=True)
    #print(continent_territories, flush=True)

    # In same continent & in_choke, may not be adjacent
    available = list(set(unclaimed_territories) & set(continent_inchoke) & set(continent_territories))
    if len(available) <= 0:
        # In same continent & adjacent
        available = list(set(unclaimed_territories) & set(adjacent_territories) & set(continent_territories))
    if len(available) <= 0:
        # In same continent, not adjacent
        available = list(set(unclaimed_territories) & set(continent_territories))
    if len(available) <= 0:
        # Not in same continent, adjacent
...
def handle_place_initial_troop(game: Game, self_state: SelfState, bot_state: BotState, query: QueryPlaceInitialTroop) -> MovePlaceInitialTroop:
    ...
    if get_continent_percent_owned(game, priority_continent) < 1.0:
        owned_in_continent = get_owned_in_continent(game, self_state, priority_continent)
...
def handle_attack(game: Game, self_state: SelfState, bot_state: BotState, query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    while len(self_state.prio_clusters) > 0 or len(self_state.cur_cluster.cluster) > 0:
        ...
        extra = []
        if self_state.cur_cluster.type == CL_TYPE_CONTINENT and len(set(self_state.cur_cluster.cluster) - set(self_state.cur_cluster.ori_cluster)) <= 0:
            continent = self_state.cur_cluster.continent
...
def get_owned_in_continent(game: Game, self_state: SelfState, continent: int) -> list[int]:
...
def generate_priority_clusters(game: Game, self_state: SelfState, deployable_troops: int) -> list[TargetCluster]:
    ...
    """
    candidate_continents = set() # Store candidate continents to target
    my_conts = get_my_continents(game)
    if len(my_conts) <= 0:
        ...
        for c in game.state.map.get_continents().keys():
            ...
            if score > best_score:
                best_score = score
                best_candidate = c
        candidate_continents.add(best_candidate)
    else:
        for cont in my_conts:
            candidate_continents = candidate_continents | set(self_state.cont_adj[cont])
        candidate_continents = candidate_continents - set(my_conts)
    """
    ...
    for continent_id in candidate_target_continents:
        ...
        #else:
            # Append cluster to attack easiest (FORCED ATTACK)
